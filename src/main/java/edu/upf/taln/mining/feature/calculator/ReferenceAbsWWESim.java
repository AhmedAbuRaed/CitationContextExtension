...
public class ReferenceAbsWWESim implements FeatCalculator<Double, TrainingExample, DocumentCtx> {
    Word2Vec aclvec;

    public ReferenceAbsWWESim(Word2Vec aclvec) {
        this.aclvec = aclvec;
    }
    ...
    public MyDouble calculateFeature(TrainingExample trainingExample, DocumentCtx documentCtx, String s) {
        ...
        for (Annotation referenceAbstractToken : referenceAbstractTokens) {
            if (referenceAbstractToken.getFeatures().get("kind").toString().equals("word")) {
                ...
                if (aclvec.hasWord(referenceAbstractToken.getFeatures().get("string").toString().toLowerCase())) {
                    referenceAbstractAVGVector = referenceAbstractAVGVector.addRowVector(aclvec.getWordVectorMatrix(referenceAbstractToken.getFeatures().get("string").toString().toLowerCase()));
        ...
        for (Annotation citSentenceToken : citSentenceTokens) {
            if (citSentenceToken.getFeatures().get("kind").toString().equals("word")) {
                if (aclvec.hasWord(citSentenceToken.getFeatures().get("string").toString().toLowerCase())) {
                    citSentenceAVGVector = citSentenceAVGVector.addRowVector(aclvec.getWordVectorMatrix(citSentenceToken.getFeatures().get("string").toString().toLowerCase()));
