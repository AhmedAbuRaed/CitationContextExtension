...
public class ReferenceAbsBCVSim implements FeatCalculator<Double, TrainingExample, DocumentCtx> {
    @Override
    public MyDouble calculateFeature(TrainingExample trainingExample, DocumentCtx documentCtx, String s) {
        ...
        AnnotationSet citSentenceNormalizedContextVectors = cp.getAnnotations("Babelnet").get("BNVector_Norm").get(citSentence.getStartNode().getOffset(), citSentence.getEndNode().getOffset());
        ...
        if ((rp.getAnnotations("Original markups").get("abstract_text") != null) && ((referenceAbstract = rp.getAnnotations("Original markups").get("abstract_text").inDocumentOrder()) != null) &&
            ...
            for (int i = 0; i < referenceAbstractSentences.size(); i++) {
                AnnotationSet normalizedContextVectors = rp.getAnnotations("Babelnet").get("BNVector_Norm").get(referenceAbstractSentences.get(i).getStartNode().getOffset(), referenceAbstractSentences.get(i).getEndNode().getOffset());
            ...
            for (int i = 0; i < 12 && i < referenceSentences.size(); i++) {
                AnnotationSet normalizedContextVectors = rp.getAnnotations("Babelnet").get("BNVector_Norm").get(referenceSentences.get(i).getStartNode().getOffset(), referenceSentences.get(i).getEndNode().getOffset());
                if (normalizedContextVectors.size() > 0) {
                    if (normalizedContextVectors.iterator().hasNext()) {
                        if (i == 0) {
                            abstractFM = normalizedContextVectors.iterator().next().getFeatures();
